...
public class Cluster implements Runnable {
	 ...
	 */
	public Cluster(String url, ArrayList<String> gridSchedulerURLs, int nodeCount, boolean restart) {
		// Preconditions
		assert (url != null) : "parameter 'name' cannot be null";
		assert (gridSchedulerURLs != null) : "parameter 'gridSchedulerURL' cannot be null";
		...
		resourceManager = new ResourceManager(this, url, gridSchedulerURLs, restart);
		resourceManager.connectToGridSchedulers();
	...
	*/
	...
	public Node getFreeNode() {
		...
		for (Node node : nodes)
			if (node.getStatus() == NodeStatus.Idle || node.getStatus() == NodeStatus.Restarted)
				return node;
	...
	public int getJobsFinished(){
		int total = 0;
		for(Node node : nodes){
			total += node.getJobsFinished();
		}
		return total;
	}

	public int getCrashedNodes(){
		return crashedNodes;
	}
