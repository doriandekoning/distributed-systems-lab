...
public class ClusterStatusPanel extends StatusPanel {
	...
	final static int panelWidth = 450;
	final static int statusTextHeight = 80;
	...
	final static Color downColor = Color.black;
    ...
    protected void paintComponent(Graphics g) {
	    ...
	    g.drawString("" + cluster.getURL(), x + colWidth, y);
	    y += fontHeight;
		...
		g.drawString("Queue size:", x, y);
		g.drawString("" + cluster.getResourceManager().getLoad(), x + colWidth, y);
		y += fontHeight;

		g.drawString("Jobs finished ", x, y);
	    g.drawString("" + cluster.getJobsFinished(), x + colWidth, y);
		y += fontHeight;
	    ...
	    for (Node node : nodes) {
	    	...
	    	if (node.getStatus() == NodeStatus.Busy) g.setColor(getBusyColor(node.getJobProgress()));
	...
	private Color getBusyColor(double progress){
		int red = (int)(255 - (255 * progress));
		if(red < 0) red = 0;
		if(red > 254) red = 254;
